...
export function ResourceCard<T extends object, C = undefined>({
  ...
  const menuEnabled = definition.menu?.enabled !== false;
  const menuItems = useMemo<ContextMenuAction[]>(
    () =>
      menuEnabled
        ...
        : [],
    [codeModal.openModal, flipped, hasBackContent, hasCode, menuEnabled, resourceMenuItems]
  );
  const menuStateContent = useResourceCardMenuState({
    menuMeta: menuEnabled ? menuMeta : null,
    menuStatus: menuEnabled ? menuStatus : null,
    status: menuEnabled ? status : null,
    typeIndicator: menuEnabled ? typeIndicator : null
  });
