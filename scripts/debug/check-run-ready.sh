...
if command -v uds >/dev/null 2>&1 && uds zarf tools kubectl cluster-info >/dev/null 2>&1; then
  ...
  printf "no   %-18s not reachable; run make deploy-uds before make up\n" "Kubernetes"
  failures=$((failures + 1))
fi
...
if [ "$package_count" -gt 0 ]; then
  ...
  printf "no   %-18s no installed Package CRs; run make deploy-uds or make deploy-uds-macos before make up\n" "UDS packages"
  failures=$((failures + 1))
fi
