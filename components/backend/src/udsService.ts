...
export function getPackageRefs(): string[] {
  return (process.env.UDS_REGISTRY_PACKAGE_REFS ?? "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
}
...
export async function getUdsStatus(): Promise<UdsStatus> {
  ...
  const packageCrs = await runPackageCrListCommand();
...
export async function getInstalledPackages(): Promise<{
  ...
  const command = await runPackageCrListCommand();
  ...
  try {
    ...
    // Source: Kubernetes Package CRs returned by kubectl/UDS CLI.
    ...
    const installedPackages = (parsed.items ?? []).map<InstalledPackage>((item) => {
      ...
      const launchEndpoints = launchEndpointsForPackage(item, endpoints);
      const namespace = item.metadata?.namespace ?? "default";
      const name = item.metadata?.name ?? "unknown";
      ...
      return {
        ...
        endpoints,
        launchEndpoints,
        launchUrl: localPocLaunchUrl(namespace, name) ?? launchUrlFromEndpoint(launchEndpoints[0]) ?? launchUrlFromPackage(item),
...
function launchEndpointsForPackage(
  item: { metadata?: { annotations?: Record<string, string | undefined> } },
  endpoints: string[]
): string[] {
  const annotation = item.metadata?.annotations?.["uds-poc.dev/launch-endpoints"]?.trim();
  ...
  const allowedHosts = new Set(
    annotation
      ...
      .filter(Boolean)
      .flatMap((value) => [value, `${value}.uds.dev`])
  );

  return endpoints.filter((endpoint) => allowedHosts.has(endpointHost(endpoint).toLowerCase()));
}
...
function localPocLaunchUrl(namespace: string, name: string): string | null {
  if (process.env.UDS_POC_LOCAL_PORT_FORWARD_URLS !== "true" || namespace !== "uds-poc") {
    return null;
  }

  if (name === "frontend") {
    return `http://localhost:${process.env.UDS_POC_FRONTEND_PORT ?? "5173"}/`;
  }

  if (name === "docs") {
    return `http://localhost:${process.env.UDS_POC_DOCS_PORT ?? "3002"}/`;
  }

  return null;
}

function runPackageCrListCommand(): Promise<CommandState> {
  if (process.env.UDS_POC_USE_KUBECTL_FOR_PACKAGE_CRS === "true") {
    return runCommand("kubectl", ["get", "package", "-A", "-o", "json"], { timeoutMs: 30_000 });
  }

  return runCommand("uds", ["zarf", "tools", "kubectl", "get", "package", "-A", "-o", "json"], {
    timeoutMs: 30_000
  });
}
...
function launchUrlFromPackage(item: {
  ...
  const host = expose.host ? hostWithDomain(expose.host) : null;
  if (!host) {
    const namespace = item.metadata?.namespace;
    const name = item.metadata?.name;
  ...
  return `${process.env.UDS_APP_URL_SCHEME ?? "https"}://${host}/`;
}
...
function hostWithDomain(host: string): string {
  if (host.includes(".")) {
    return host;
  }

  return `${host}.${process.env.UDS_APP_DOMAIN ?? "uds.dev"}`;
}
...
export async function publishPackage({
  ...
  return {
    ...
    packageRef,
    result: null,
    error: `Publishing ${requestedName} is a host-side Make/Zarf operation. Run make publish-catalog-poc or make publish-docs before installing from the UI.`
  };
}
...
function mutableRegistryRefPattern({ exact = false }: { exact?: boolean } = {}): RegExp {
  const repositoryPath = `${catalogPocRepository()}/[^/:]+`;
  const end = exact ? "(?<tag>[^/]+)$" : "";

  return new RegExp(`^oci:\\/\\/(?<registry>(localhost|127\\.0\\.0\\.1|host\\.k3d\\.internal):\\d+)\\/(?<repository>${repositoryPath}):${end}`);
}
...
async function deleteLocalRegistryManifest(ref: string): Promise<CommandState> {
  ...
  if (!parsed) {
    return {
      ...
      command,
      stdout: "",
      stderr: `Only local ${catalogPocRepository()} refs can be unpublished.`,
      exitCode: null
    };
  }
...
function isMutableInstalledPackage({ name, namespace }: { name: string; namespace: string }): boolean {
  ...
  return isLocalPocAppPackage(name, namespace);
}
...
function localOciFlags(ociReference: string): string[] {
  ...
  return /^oci:\/\/(localhost|127\.0\.0\.1|host\.k3d\.internal):/.test(ociReference) ? ["--plain-http"] : [];
}

function isLocalPocAppPackage(name: string, namespace: string): boolean {
  return [catalogPocName(), "docs"].some((packageName) => name === packageName && namespace === packageName);
}
