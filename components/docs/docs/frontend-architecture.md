...
Current renderer families live under `components/frontend/src/components` by base rendering pattern:
...
Feature-specific files live under `components/frontend/src/features` and should stay thin: select data, pass business context, and choose renderer definitions.
